   
   <!-- footer area start -->
    <footer>
        <div class="footer-top has-color pt--120 pb--30">
            <div class="container">
                <div class="row">
                    <div class="col-md-4">
                        <div class="widget widget-company">
                            <a href="index.html"><img src="assets/images/icon/atmc_logo.png" alt="image"></a>
                            <div class="address">
                                <h6>ATMC ADDRESS</h6>
                                <p>399 Lonsdale Street, Melbourne VIC, Australia</p>
                            </div>
                            <div class="address">
                                <h6>BUSINESS PHONE</h6>
                                <p>+012 3456 7890</p>
                            </div>
                            <div class="address">
                                <h6>BUSINESS EMAIL</h6>
                                <p>hello@employability.life</p>
                            </div>
                            <ul class="social">
                                <li><a href="https://www.facebook.com/employabilitylife"><i class="fa fa-facebook"></i></a></li>
                                <li><a href="https://twitter.com/Emplifenetwork"><i class="fa fa-twitter"></i></a></li>
                                <li><a href="#"><i class="fa fa-dribbble"></i></a></li>
                                <li><a href="https://www.instagram.com/employabilitylife/"><i class="fa fa-instagram"></i></a></li>
                            </ul>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="widget footer-link">
                            <h4 class="fwidget-title mb-5 pb-3 primary-color">Footer Menu</h4> 
                            <ul>
                                <li><a href="course_details.php"><i class="fa fa-arrow-right"></i>Our courses</a></li>
                                <li><a href="about.php"><i class="fa fa-arrow-right"></i>About Us</a></li>
                                <li><a href="news.php"><i class="fa fa-arrow-right"></i>News</a></li>
                                <li><a href="register.php"><i class="fa fa-arrow-right"></i>Enrollment</a></li>
                                <li><a href="team.php"><i class="fa fa-arrow-right"></i>Our Team</a></li>
                            </ul>
                        </div>
                    </div>
                    <div class="col-md-5">
                        <div class="widget widget-opening">
                            <h4 class="fwidget-title mb-5 pb-3 primary-color">Working Day & time</h4>
                            <p>We are looking forward to meeting you online or on campus!</p>
                           <ul>
                                <li><span>Monday - Thursday :</span>9:00 am - 7:00 pm</li>
                                <li><span>Friday :</span>9:00 am - 5:00 pm</li>
                                <li><span>Saturday - Sunday :</span>Closed</li>
                            </ul>
                        </div>
                    </div>
                </div>
                <div class="footer-bottom">
                    <p>Copyright © 2022 Employability Life - All Rights Reserved. </p>
                </div>
            </div>
        </div>
    </footer>
    <!-- footer area end -->


    <!-- jquery latest version -->
    <script src="assets/js/vendor/jquery-2.2.4.min.js"></script>
    <!-- bootstrap 4 js -->
    <script src="assets/js/bootstrap.min.js"></script>
    <!-- others plugins -->
    <script src="assets/js/owl.carousel.min.js"></script>
    <script src="assets/js/jquery.magnific-popup.min.js"></script>
    <script src="assets/js/jquery.slicknav.min.js"></script>
    <script src="assets/js/plugins.js"></script>
    <script src="assets/js/scripts.js"></script>
	
	

</body>

</html>